...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio = 0.5):
        ...
        trgmask = np.stack(np.zeros(max_len) for i in range(max_len))
        ...
        input = embedded_trg[0].unsqueeze(0)
        
        for t in range(1, max_len):            
            output = self.transformer(embedded_src, input, trgmask)
            #output = self.transformer(embedded_src, input)
            outputs[t] = output
            teacher_force = random.random() < teacher_forcing_ratio
            top1 = output
            input = torch.stack((input,embedded_trg[t] if teacher_force else top1))
